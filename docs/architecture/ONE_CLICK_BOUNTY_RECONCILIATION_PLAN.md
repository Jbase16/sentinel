...
| Issued-capability contracts through injected effect evaluation and cleanup | R5D1-R5D7 are suite-proved at their bounded passive, local-storage, expiry, and receipt layers; R5D8's default-off injected executor, independent oracle, and verified-cleanup boundary are suite-proved; all remain production-unwired | R5D1-R5D7 retain the content-addressed authority, confinement/freshness, immutable and durable consumption, runtime-expiry, terminal-receipt, and durable-receipt layers. R5D8 consumes one exact completed R5D6 receipt, binds the authorized witness and four refusal outcomes to the existing owned experiment world, drives the fixed matrix only through an injected transport, hashes rather than retains target effects, and fails closed unless cleanup is verified. The same oracle confirms the secure twin and refutes the leaking twin. It has no concrete client, production caller, or finding-promotion authority. The full suite is `3150 passed, 1 skipped, 2 warnings in 35.85s`; the two unchanged dependency warnings remain, while the two known scheduling-sensitive `aiosqlite` warnings from R5D7 did not occur. No warning is attributable to R5D8. | Add finding-promotion/durable-claim and production-wiring design, then run an exact-SHA lab matrix and `OCB-S17`; no later slice identifier is assigned. |
...
- [x] R5C8 at Sentinel `8329be8` restores exact
...
- [x] **R5C9 — renew current-SHA acceptance.** The R5D1 work-order handoff records the
  negative protocol gate and native vulnerable/secure matrix as passed and
  ...
  complete SentinelForge app and its own `:8766` backend run against the lab website,
  while the lab observes the authenticated public API plus target ledger and attests
  the Sentinel SHA. No app, package, backend, sidecar, source, or verifier change
  crosses the repository boundary. Broader membership-creation, role-assignment, and
  administrative-boundary shapes stay **open and deferred**; invitation-token/link
  confinement remains Family D. None was absorbed into R5C9.

#### R5D — Capability confinement, freshness, and replay

- [x] R5D starts only after the R5C9 **acceptance gate**. That gate is recorded as
  mediator-verified above. Invitation token/link confinement belongs here; membership
  creation, role assignment, and resulting authority remain **open, deferred**
  Family-C obligations.
- [x] R5D1 defines a transport-free, content-addressed issued-capability contract,
  controlled single-owned-account fixture, and pure five-outcome classifier before any
  executable backend exists. The detailed design and precedence rationale are in
  [`ONE_CLICK_BOUNTY_R5D1_CAPABILITY_CONTRACT_PLAN.md`](ONE_CLICK_BOUNTY_R5D1_CAPABILITY_CONTRACT_PLAN.md).
- [x] R5D2 defines a transport-free confined-presentation evidence binding over the
  exact R5D1 capability, owned world, tenant, canonical origin, and structurally
  revalidated current capture. Its pure evaluator deterministically classifies
  confined-fresh, escaped-confinement, or stale-capture evidence with confinement
  escape first. The detailed contract is in
  [`ONE_CLICK_BOUNTY_R5D2_CONFINEMENT_FRESHNESS_PLAN.md`](ONE_CLICK_BOUNTY_R5D2_CONFINEMENT_FRESHNESS_PLAN.md).
- [x] R5D3 defines a transport-free immutable consumption ledger and pure evaluator
  over already-admissible R5D1 and R5D2 decisions. Its fixed precedence is identical-
  presentation replay, then distinct-presentation capability exhaustion, then first
...
- [x] R5D5 defines wall-clock expiry semantics under an explicit, content-addressed
  admitted-runtime contract. Its pure evaluator preserves every non-`VALID` R5D1
  refusal, otherwise classifies an injected finite epoch against a left-closed,
  right-open absolute window, and remains production-unwired. It reads no real clock,
  performs no I/O, and owns no dispatch or receipt authority. The detailed contract is
  in
  [`ONE_CLICK_BOUNTY_R5D5_RUNTIME_EXPIRY_PLAN.md`](ONE_CLICK_BOUNTY_R5D5_RUNTIME_EXPIRY_PLAN.md).
- [x] R5D6 reads one real wall-clock instant and mints an ephemeral,
  content-addressed terminal execution-receipt decision. It delegates all liveness
  classification to R5D5, consumes R5D3's already-computed verdict without mutating or
  persisting it, and uses the fixed inadmissible, already-consumed, expired, not-live,
  completed precedence. It remains production-unwired, sends no target traffic, and
  owns no effect, finding, cleanup, or durable-receipt authority. The detailed contract
  is in
  [`ONE_CLICK_BOUNTY_R5D6_EXECUTION_RECEIPT_PLAN.md`](ONE_CLICK_BOUNTY_R5D6_EXECUTION_RECEIPT_PLAN.md).
- [x] R5D7 durably persists R5D6's exact public receipt projection in a purpose-built
  append-only local namespace and reloads it as a distinct inert value. It reuses the
  audited receipt store's exclusive hard-link and `fsync` boundary, binds filename,
  content, and recomputed receipt identity, persists all five outcomes, and treats
  identical concurrent writes as idempotent convergence rather than a first-execution
  race. It reads no clock, remains production-unwired, and observes no target. The
  detailed contract is in
  [`ONE_CLICK_BOUNTY_R5D7_EXECUTION_RECEIPT_STORE_PLAN.md`](ONE_CLICK_BOUNTY_R5D7_EXECUTION_RECEIPT_STORE_PLAN.md).
- [x] R5D8 consumes one exact completed R5D6 receipt, binds the authorized witness and
  no-capability, replayed, expired, and inadmissible refusals to exact R5D6 terminal
  outcomes and the existing owned world, and drives those observations only through
  an injected async transport. Its independent content-addressed oracle confirms one
  authorized effect or refutes leakage/duplication, and verified cleanup is mandatory.
  It remains default-off and production-unwired, with no concrete client, clock read,
  or finding-promotion authority. The detailed contract is in
  [`ONE_CLICK_BOUNTY_R5D8_CAPABILITY_EFFECT_ORACLE_PLAN.md`](ONE_CLICK_BOUNTY_R5D8_CAPABILITY_EFFECT_ORACLE_PLAN.md).
- [ ] Prove object-bound, account-bound, tenant-bound, single-use, and expiry properties
  through an externally attested admitted runtime. R5D1-R5D7 define and persist the
  logical, evidence, consumption, expiry, and receipt layers. R5D8 now drives dispatch
  only through an injected transport, is unit-tested against in-process loopback twins,
  and remains production-unwired; any lab attestation must be separately run and
  SHA-bound. No Family-D module observes a target by itself or promotes a finding.
...
- [x] R5D5 adds expiry semantics only: an admitted absolute wall-clock window and pure,
  clock-injected liveness evaluation. It does not read or enforce a target-relevant
  clock.
- [x] R5D6 adds one real-clock read and an ephemeral terminal execution-receipt
  decision only. It does not persist the receipt, dispatch an action, evaluate an
  effect, promote a finding, or verify cleanup.
- [x] R5D7 durably persists capability-execution receipt public projections in a
  purpose-built namespace with cross-process idempotent convergence, restart survival,
  refusal immutability, and tamper-evident reload while remaining production-unwired.
  Its source and R5D6 source remain unchanged by R5D8.
- [x] R5D8 adds the independent five-phase capability-effect oracle, default-off
  injected transport execution, exact R5D6 refusal binding, and fail-closed verified
  cleanup. Its confirmed result is a triage-required finding candidate only;
  `promotion_authority` and `finding_authority` remain false.
- [ ] Accept `OCB-S17` for confinement, expiry, one-time use, replay refusal, and
  cleanup. No R5D1-R5D8 unit or suite result satisfies this external gate.

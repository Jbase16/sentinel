...
| Issued-capability logical and evidence contracts | Passive only; R5D1 and R5D2 suite-proved | R5D1 binds the content-addressed authority tuple and returns a pure logical outcome. R5D2 separately binds the exact owned world, tenant, canonical origin, and structurally revalidated current capture, then purely classifies confined-fresh, escaped, or stale evidence with escape-first precedence. Public evidence is hash-only, and the full suite is `2842 passed, 1 skipped, 3 warnings`. | Add a separately admitted backend, durable one-time consumption and cross-presentation replay refusal, running-target expiry, effect proof, receipts, cleanup, and `OCB-S17`. |
...
- [ ] Prove object-bound, account-bound, tenant-bound, single-use, and expiry properties
  through a later admitted runtime. R5D1 types logical bounds and R5D2 classifies
  operator-supplied evidence, but neither enforces or observes them against a target.
...
- [ ] Accept `OCB-S17` for confinement, expiry, one-time use, replay refusal, and
  cleanup. No R5D1 or R5D2 unit or suite result satisfies this external gate.

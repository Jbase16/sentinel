...
R5D2 source baseline: clean `main` at
`2bf2e1b0ae900f48b9c1f0a9240c4d629e65ec95`. R5D1 is suite-proved at that base; its
work-order handoff records the R5C9 current-SHA acceptance renewal as passed and
mediator-verified on 2026-08-26 against the unchanged verifier. R5D2 adds only the
passive confined-presentation evidence binding and pure evaluator described below.
...
plus deterministic offline evaluator. Neither slice is production-wired. Payout
...
- R5D2 source baseline: clean `main` at
  `2bf2e1b0ae900f48b9c1f0a9240c4d629e65ec95`; Sentinel-only R5C8 implementation
...
- `scripts/local-security-check.sh` remains red on repository-wide broad-text matchers
  and Ruff debt outside R5D2. Its R5D2 checkpoint run exits 1 on that documented
  baseline; targeted Ruff passes every changed R5D2 Python file.
...
| OCB-R5 family D: capability safety | R5D1 and R5D2 suite-proved, passive, and unwired | R5D1 adds the issued-capability contract and logical classifier. R5D2 binds its evidence axis to an exact owned world, tenant, canonical origin, and structurally revalidated current capture, then purely classifies confined-fresh, escaped, or stale evidence. No executable capability backend exists. | Add durable one-time/replay and running-target expiry enforcement, effect proof, receipts, cleanup, and `OCB-S17` |
...
- [ ] Add durable one-time consumption, cross-presentation replay refusal, expiry
  enforcement against a running target, receipt completion/abort semantics, verified
  cleanup, and an independent target-effect oracle.
- [ ] Run and accept `OCB-S17` for confinement, expiry, one-time use, replay refusal,
  and cleanup. No R5D1 or R5D2 unit result closes this gate.
...
R5D2 is the current implementation slice. It adds only the passive typed
confinement/current-capture evidence binding and pure three-outcome evaluator described
in its detailed plan. The planned next Family-D work is a separately scoped durable
one-time-consumption and replay-refusal slice, followed by running-target expiry
handling, effect proof, receipts, cleanup, and `OCB-S17`. No later slice identifier is
...
| [ ] | OCB-R5 family D capabilities | R5D1 logical contract/classifier and R5D2 confinement/current-capture evidence binding/evaluator are suite-proved, passive, and unwired; no executable backend or `OCB-S17` proof exists | Separately scope durable one-time/replay, running-target expiry, effect proof, receipts, cleanup, and `OCB-S17` |

...
def test_event_sequence_never_decreases_across_restarts():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        async def run_test():
            # --- Phase 1: First "boot" ---
            # Reset global state WITHOUT re-initializing, so
            # initialize_event_sequence_from_db() can do a clean DB init.
            ...
            test_config = SentinelConfig(
                storage=StorageConfig(base_dir=Path(tmpdir), db_name="test_sequences.db"),
            ...
            try:
                ...
                last_seq = GlobalSequenceAuthority.instance().last_issued
                ...
                # Reset the in-memory singleton state (without re-initializing)
                Database._instance = None
                GlobalSequenceAuthority.reset_for_testing()
...
def test_event_sequence_raises_if_not_initialized():
    ...
    """
    # Reset global state WITHOUT re-initializing (reset_event_sequence re-inits)
    GlobalSequenceAuthority.reset_for_testing()
    reset_run_id()
    ...
    try:
        ...
        # Re-initialize so other tests aren't affected

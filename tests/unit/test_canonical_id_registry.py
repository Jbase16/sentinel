...
def test_registry_accepts_historical_aliases_and_rejects_new_bare_ids() -> None:
    ...
    with pytest.raises(IdentifierRegistryError, match=r"bare or unregistered IDs: R1, S15"):
...
def test_registry_rejects_colliding_canonical_ids() -> None:
    payload = json.loads(default_registry_path(REPOSITORY_ROOT).read_text(encoding="utf-8"))
...
def test_registry_accepts_registered_family_slice_ids() -> None:
    ...
    assert {"R5D1", "R5D2", "R5D3", "R5D4"} <= registry.canonical_ids
